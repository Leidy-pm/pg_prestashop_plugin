...
class PG_Prestashop_PluginLtpModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        PG_Prestashop_Utils::markOrderPaymentFlow($order, 'ltp');
    ...
    private function callLtpInitOrder(
        ...
        $ltp_url = ($environment === 'stg')
            ? 'https://noccapi-stg.' . FLAVOR_DOMAIN . '/linktopay/init_order/'
            : 'https://noccapi.' . FLAVOR_DOMAIN . '/linktopay/init_order/';
        ...
        if ($response === false || $curl_error !== '') {
            PrestaShopLogger::addLog('Paymentez linktopay cURL error: ' . $curl_error, 3);
            return ['success' => false, 'error' => 'LinkToPay communication failed'];
        }

        if ($http_code >= 400) {
            PrestaShopLogger::addLog('Paymentez linktopay failed. HTTP ' . $http_code . ' Response: ' . $response, 3);
        }
    ...
    private function getBillingAddress(Cart $cart, Customer $customer): array
    {
        ...
        if (Validate::isLoadedObject($billingAddress)) {
            ...
            $countryIso = PG_Prestashop_Utils::get_convert_country((string)$country->iso_code);
        }

        $stateIso = '';
        if (Validate::isLoadedObject($billingAddress) && (int)$billingAddress->id_state > 0) {
            $state = new State((int)$billingAddress->id_state);
            $stateIso = PG_Prestashop_Utils::validate_state((string)$state->iso_code);
        }
